...
class User private constructor(
    ...
    init {
        println("Firs init block,primary constructor was called")
    ...
    fun changePassword(oldPass: String, newPass: String) {
        if (checkPassword(oldPass)) {
            ...
            if (!accessCode.isNullOrEmpty()) {
                accessCode = newPass
                println("Password $oldPass has been changed on new $newPass")
            }
            println("Password $oldPass does not been changed on new $newPass")
    ...
    companion object Factory {
        ...
        private fun String.fullNameToPair(): Pair<String, String?> =
            this.split(" ").filter { it.isNotBlank() }.run {
                when (size) {
                    1 -> first() to null
                    2 -> first() to last()
                    else -> throw java.lang.IllegalArgumentException("Full name contain only first name and last name, current split" + "result: ${this@fullNameToPair}")
                }
            }

        fun normalizePhone(value: String?): String? {
            val tmp = value?.replace("""[^+\d]""".toRegex(), "")

            return when {
                Regex(pattern = "^\\+\\d{11}").matches(tmp.toString()) -> tmp
                else -> null
            }
        }
